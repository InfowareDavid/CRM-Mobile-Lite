...
class NationalityViewController: AgeGroupViewController {
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor.white
        controllerStyle = infControllerStyle.nationalityMaster
        self.updateNationalityView()
        dataArray = dbManager.searchAllNationality()
        self.ageGroupView.masterTableView.reloadData()
    }
    
    func updateNationalityView(){
        self.ageGroupView.contentLabel.text = "natNationality".localString()
        self.ageGroupView.basePageLabel.text = "natNationalityMaster".localString()
    }
    ...
    override func addAction(){
        if lastSelectedCell?.contentTextField.text != "" || self.dataArray.count == 0{
            ...
            if  self.dbManager.addNationality(model){
    ...
    override func deleteAction(){
        ...
        if self.currentIndexPath != nil && self.currentIndexPath?.row < self.dataArray.count{
            ...
            self.ageGroupView.masterTableView.reloadData()
             _ = self.dbManager.deleteNationality(model)
        ...
        if lastSelectedCell?.contentTextField.text != "" {
            ...
            if ageGroupModel?.contentStr != textField.text {
                if textField.text != "" {
                    ...
                    if !dbManager.updateNationality(ageGroupModel){
                        self.dataArray.removeObject(at: (indexPath?.row)!)
                        ageGroupModel?.contentStr = ""
                        _ =  self.dbManager.deleteNationality(ageGroupModel)
                        //弹出提示框 提示重复
                    }else{
