...
class OccupationViewController: AgeGroupViewController {
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor.white
        controllerStyle = infControllerStyle.occupationMaster
        self.updateOccupationView()
        dataArray = dbManager.searchAllOccupation()
        self.ageGroupView.masterTableView.reloadData()
    }
    
    func updateOccupationView(){
        self.ageGroupView.contentLabel.text = "occOccupation".localString()
        self.ageGroupView.basePageLabel.text = "occOccupationMaster".localString()
    }
    //MARK: - ButtonAction
    ...
    override func addAction(){
        if lastSelectedCell?.contentTextField.text != "" || self.dataArray.count == 0{
            ...
            model.contentStr = ""
            if self.dbManager.addOccupation(model){
    ...
    override func deleteAction(){
        ...
        if self.currentIndexPath != nil && self.currentIndexPath?.row < self.dataArray.count{
            ...
            self.ageGroupView.masterTableView.reloadData()
            _ = self.dbManager.deleteOccupation(model)
        ...
        if lastSelectedCell?.contentTextField.text != "" {
            ...
            if ageGroupModel?.contentStr != textField.text {
                if textField.text != "" {
                    ...
                    if !dbManager.updateOccupation(ageGroupModel){
                        self.dataArray.removeObject(at: (indexPath?.row)!)
                        ageGroupModel?.contentStr = ""
                         _ = self.dbManager.deleteOccupation(ageGroupModel)
                        //弹出提示框 提示重复
                    }else{
